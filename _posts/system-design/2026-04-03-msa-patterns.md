...
- **여러 서비스에 걸친 트랜잭션은?** → DB 트랜잭션으로 묶을 수 없다 ([트랜잭션 격리 수준](/2026/04/06/database-transaction-isolation/) 참고)
- **외부 클라이언트는 어디로 요청해야 하는가?** → 서비스별 엔드포인트를 다 알아야 한다
- **서비스 간 통신은 REST로 충분한가?** → 내부 통신은 [gRPC](/2026/04/03/grpc-spring-boot/)가 성능과 타입 안정성에서 유리하다
...
| **Rate Limiting** | [Rate Limiting 전략](/2026/04/01/api-rate-limiting/)을 중앙에서 적용 |
| **로드밸런싱** | 동일 서비스의 여러 인스턴스에 트래픽 분산 |
| **응답 캐싱** | 자주 요청되는 데이터를 [캐시](/2026/03/28/caching-strategy/)하여 백엔드 부하 감소 |
...
> **참고**: [Kubernetes 환경](/2026/04/01/kubernetes-basics/)에서는 k8s Service가 Service Discovery 역할을 하므로 Eureka 없이도 동작한다. 다만 Spring Cloud와의 통합이 필요하면 Eureka 또는 Consul을 함께 사용하기도 한다.
...
[Kafka](/2026/04/03/kafka-introduction/)가 이벤트 전달 채널로 자주 사용된다.
...
이 패턴들의 효과를 극대화하려면 비동기 메시징([Kafka](/2026/04/03/kafka-introduction/))과 적절한 캐싱 전략([캐싱 패턴](/2026/03/28/caching-strategy/))을 함께 설계하는 것이 중요하다.

---

## 관련 포스트

- [Apache Kafka 입문부터 실전까지](/2026/04/03/kafka-introduction/)
- [API Rate Limiting — 설계와 구현 전략](/2026/04/01/api-rate-limiting/)
- [캐싱 전략 — Cache-Aside, Write-Through, Write-Behind](/2026/03/28/caching-strategy/)
