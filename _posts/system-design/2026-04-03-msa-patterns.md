...
- **여러 서비스에 걸친 트랜잭션은?** → DB 트랜잭션으로 묶을 수 없다 ([트랜잭션 격리 수준](/database/2026/04/06/database-transaction-isolation/) 참고)
- **외부 클라이언트는 어디로 요청해야 하는가?** → 서비스별 엔드포인트를 다 알아야 한다
- **서비스 간 통신은 REST로 충분한가?** → 내부 통신은 [gRPC](/infra/2026/04/03/grpc-spring-boot/)가 성능과 타입 안정성에서 유리하다
