...
- 마이크로서비스 간 내부 통신 (특히 [Kubernetes 환경](/2026/04/01/kubernetes-basics/)에서)
- 실시간 양방향 통신이 필요한 서비스
...
실무에서는 **외부 API Gateway는 REST**, **내부 서비스 메시는 gRPC**로 구성하는 하이브리드 패턴이 가장 많이 사용된다. API Gateway 설계에 대해서는 [API Rate Limiting](/2026/04/01/api-rate-limiting/) 포스트도 참고하자.
...
[Kafka](/2026/04/03/kafka-introduction/)와 gRPC를 조합하면, Kafka는 비동기 이벤트 기반 통신을, gRPC는 동기 서비스 간 호출을 담당하는 견고한 마이크로서비스 통신 아키텍처를 구성할 수 있다.
...
- [Docker 입문](/2026/03/20/docker-getting-started/)
