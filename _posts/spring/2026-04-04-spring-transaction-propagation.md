...
[이전 글(Spring AOP 내부 동작)](/2026/04/03/spring-aop-internals/)에서 `@Transactional`이 AOP 프록시를 통해 동작하는 원리를 살펴봤다. 이번에는 그 안에서 트랜잭션 전파가 어떻게 결정되는지 깊이 파고들어 본다.
...
1. **Self-invocation에서는 전파 레벨이 무시된다** — [AOP 내부 동작](/2026/04/03/spring-aop-internals/) 글에서 다룬 것처럼, 같은 클래스 내부 호출은 프록시를 거치지 않으므로 전파 레벨 설정이 아무 의미가 없다.
...
트랜잭션 전파 레벨은 "어떤 상황에서 어떤 레벨을 쓰는지" 보다 **"각 레벨이 실제로 어떻게 동작하는지"** 를 이해하는 것이 중요하다. 전파 레벨과 함께 [데이터베이스 격리 수준](/2026/04/06/database-transaction-isolation/)을 이해하면 동시성 문제에 대한 전체 그림이 완성된다. 특히 `REQUIRED`에서의 rollback-only 마크, `REQUIRES_NEW`의 커넥션 사용량, 체크드 예외의 기본 롤백 규칙은 실무에서 정말 자주 문제가 되는 부분이니 꼭 기억해 두자.
