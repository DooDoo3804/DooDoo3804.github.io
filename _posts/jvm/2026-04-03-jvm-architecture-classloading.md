...
> 다음 편: [GC 종류와 튜닝 전략](/2026/04/04/jvm-gc-tuning/)
...
Heap 영역이 바로 [GC 튜닝](/2026/04/04/jvm-gc-tuning/)과 [힙 덤프 분석](/2026/04/05/jvm-heap-dump-analysis/)의 핵심 대상이다.
...
다음 포스트에서는 Runtime Data Areas 중 Heap 영역을 관리하는 핵심 메커니즘인 [Garbage Collection과 튜닝 전략](/2026/04/04/jvm-gc-tuning/)을 다룬다. 참고로, Spring 프레임워크에서 Bean이 생성되고 프록시가 만들어지는 과정도 이 클래스 로딩 메커니즘 위에서 동작한다. [Spring Bean 라이프사이클](/2026/04/05/spring-bean-lifecycle/)에서 그 과정을 확인할 수 있다.
