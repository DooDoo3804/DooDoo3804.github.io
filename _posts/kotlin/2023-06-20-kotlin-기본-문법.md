...
author: "DoYoon Kim"
header-style: text
header-bg-css: "linear-gradient(135deg, #7b2ff7 0%, #f107a3 100%)"
header-mask: 0.4
catalog: true
keywords: "kotlin, basic syntax, variable, function"
series: "Kotlin 마스터"
tags:
  - Kotlin
categories:
  - kotlin
description: "Kotlin 기본 문법 총정리. 함수, var/val 변수, 조건식, when, 반복문, Array와 List, Nullable 처리, 클래스 상속까지 핵심 문법을 예제 코드와 함께 설명합니다."
---
...
kotlin에서는 변수의 타입을 자동으로 추론하기 때문에 `c`에서 처럼 `: Int`를 적지 않아도 가능하다. 이는 Int 에서만 적용되는 것이 아니라 모두 적용된다.(s의 경우)
